...
public class ARAVRInput
{
    ...
    static IEnumerator VibrationCoroutine(float duration, float frequency, float amplitude, Controller hand)
    {
     ...
     // 컨트롤러에 진동 호출하기
     // duration: 지속시간, frequency: 빈도, amplitude: 진폭, hand: 왼쪽 또는 오른쪽 컨트롤러
...
#elif Oculus
    private static Vector3 originScale = Vector3.one * 0.005f;    
#endif
     ...
     public static void DrawCrosshair(Transform crosshair, bool isHand = true, Controller hand = Controller.RTouch)
     {
         ...
         {
            // 레이의 GetPoint 메서드를 이용해 충돌 지점의 위치를 가져온다.
         ...
         {
             crosshair.position = ray.origin + ray.direction * distance;
             crosshair.forward = -Camera.main.transform.forward;
